...
int isPointInPolygon(const Point& p, const Polygon& poly){
  ...
  for(int i = 0; i < n; i++){
    ...
    if(p1 == p || p2 == p || OnSegment(p, p1, p2)) return -1; // 在邊界上
...
bool isDiagonal(const Polygon& poly, int a, int b) {
  int n = poly.size();
  for(int i = 0; i < n; i++)
    if(i != a && i != b && OnSegment(poly[i], poly[a], poly[b])) return false; // 中間不能有其他點
  for(int i = 0; i < n; i++)
    if(SegmentProperIntersection(poly[i], poly[(i+1)%n], poly[a], poly[b])) return false; // 不能和多邊形的邊規範相交
  Point midp = (poly[a] + poly[b]) * 0.5;
  return (isPointInPolygon(midp, poly) == 1); // 整條線段在多邊形內
}
